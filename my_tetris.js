...
// in ru lang
// Монитор производительности JavaScript
// Этот класс предоставляет простое информационное окно, которое поможет вам отслеживать производительность вашего кода.

// FPS Кадры, отображаемые за последнюю секунду. Чем выше число, тем лучше.
// MS Миллисекунды, необходимые для рендеринга кадра. Чем меньше число, тем лучше.
...
var Stats=newFunction();

function newFunction() {
    return function () {
        function s(a, g, d) {
            var f, c, e; for (c = 0; c < 30; c++)
                for (f = 0; f < 73; f++)
                    e = (f + c * 74) * 4, a[e] = a[e + 4], a[e + 1] = a[e + 5], a[e + 2] = a[e + 6]; for (c = 0; c < 30; c++)
        ...
        h.appendChild(i); a = document.createElement("canvas"); a.width = 74; a.height = 30; a.style.display = "block"; a.style.marginLeft = "3px"; h.appendChild(a); q = a.getContext("2d"); q.fillStyle = "#301010"; q.fillRect(0, 0, a.width, a.height); E = q.getImageData(0, 0, a.width, a.height); return {
            domElement: g, update: function () {
                u++; j = (new Date).getTime(); n = j - F; z = Math.min(z, n); A = Math.max(A, n); s(B.data, Math.min(30, 30 - n / 200 * 30), "ms"); c.innerHTML = '<span style="font-weight:bold">' + n + " MS</span> (" + z + "-" + A + ")"; o.putImageData(B, 0, 0); F = j; if (j >
                    v + 1E3) {
                        l = Math.round(u * 1E3 / (j - v)); w = Math.min(w, l); x = Math.max(x, l); s(y.data, Math.min(30, 30 - l / 100 * 30), "fps"); d.innerHTML = '<span style="font-weight:bold">' + l + " FPS</span> (" + w + "-" + x + ")"; m.putImageData(y, 0, 0); if (t == 3)
                            p = performance.memory.usedJSHeapSize * 9.54E-7, C = Math.min(C, p), D = Math.max(D, p), s(E.data, Math.min(30, 30 - p / 2), "mb"), i.innerHTML = '<span style="font-weight:bold">' + Math.round(p) + " MB</span> (" + Math.round(C) + "-" + Math.round(D) + ")", q.putImageData(E, 0, 0); v = j; u = 0;
                }
            }
        };
    };
